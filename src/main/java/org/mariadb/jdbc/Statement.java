...
public class Statement implements java.sql.Statement {
   ...
   *
   * <p>MariaDB specific: {@code Integer.MIN_VALUE} means sequential access. Forward-only read-only
   * result-sets then read rows from the socket one column at a time, so a Blob, Clob, InputStream
   * or Reader column is streamed without being loaded in memory. Columns must be read in increasing
   * index order, and a streamed column can be read only once. See {@link
   * org.mariadb.jdbc.client.result.SequentialResult}.
   *
   * @param rows the number of rows to fetch, or {@code Integer.MIN_VALUE} for sequential access
   * @throws SQLException if a database access error occurs, this method is called on a closed
   *     <code>Statement</code> or the condition {@code rows >= 0} (or {@code Integer.MIN_VALUE}) is
   *     not satisfied.
   * @see #getFetchSize
   * @since 1.2
   */
