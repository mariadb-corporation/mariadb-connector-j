...
public class Connection implements java.sql.Connection {
  ...
  public Connection(Configuration conf, ClosableLock lock, Client client) {
    ...
    this.canCachePrepStmts = conf.cachePrepStmts() && conf.prepareThreshold() >= 0;
    this.defaultFetchSize = conf.defaultFetchSize();
  }

  /**
   * Internal method. Indicate that a connection is created from an internal pool
   *
  ...
  public PreparedStatement prepareStatement(String sql) throws SQLException {
    return prepareInternal(
        sql, Statement.NO_GENERATED_KEYS, ResultSet.TYPE_FORWARD_ONLY, ResultSet.CONCUR_READ_ONLY);
  }
  ...
  public PreparedStatement prepareInternal(
      String sql, int autoGeneratedKeys, int resultSetType, int resultSetConcurrency)
      throws SQLException {
    checkNotClosed();

    // Use client-side preparation if not binary mode
    if (conf.prepareThreshold() == -1) {
    ...
    // Binary mode without caching - use server-side preparation
    if (!canCachePrepStmts) {
    ...
    // Binary mode with caching - check hot cache and execution count
    String cacheKey = client.getContext().getDatabase() + "|" + sql;
    PrepareCache cache = client.getContext().getPrepareCacheInstance();

    // Check hot cache first
    CachedPrepareResultPacket prepareResult = cache.get(cacheKey);
    if (prepareResult != null) {
      // Already in hot cache - increment use count and return
      prepareResult.incrementUse();
      return new ServerPreparedStatement(
          NativeSql.parse(sql, client.getContext()),
          this,
          lock,
    ...
    // Not in hot cache - check if execution count has reached threshold
    if (cache.incrementExecutionCount(cacheKey)) {
      // Threshold reached - use server-side preparation
      return new ServerPreparedStatement(
          NativeSql.parse(sql, client.getContext()),
          this,
          lock,
    ...
    // Below threshold - use client-side preparation
    return new ClientPreparedStatement(
        NativeSql.parse(sql, client.getContext()),
        this,
        lock,
  ...
  public PreparedStatement prepareStatement(String sql, int resultSetType, int resultSetConcurrency)
      throws SQLException {
    return prepareInternal(
        sql, Statement.RETURN_GENERATED_KEYS, resultSetType, resultSetConcurrency);
  }
  ...
  public PreparedStatement prepareStatement(String sql, int autoGeneratedKeys) throws SQLException {
    return prepareInternal(
        sql, autoGeneratedKeys, ResultSet.TYPE_FORWARD_ONLY, ResultSet.CONCUR_READ_ONLY);
  }
  ...
  public boolean cachePrepStmts() {
    return canCachePrepStmts;
  }

  public boolean useServerMaxRows() {
    return canUseServerMaxRows;
  }
