...
public class Connection implements java.sql.Connection {
  ...
  /**
   * Creates a {@code Statement} object that will generate {@code ResultSet} objects with the given
   * type and concurrency. This method is the same as the {@code createStatement} method above, but
   * it allows the default result set type and concurrency to be overridden. The holdability of the
   ...
   *     ResultSet.TYPE_SCROLL_INSENSITIVE}, or {@code ResultSet.TYPE_SCROLL_SENSITIVE}, or {@code
   *     MariaDbResultSet.TYPE_SEQUENTIAL_ACCESS_ONLY}, or
   * @param resultSetConcurrency a concurrency type; one of {@code ResultSet.CONCUR_READ_ONLY} or
   *     {@code ResultSet.CONCUR_UPDATABLE}
   * @return a new {@code Statement} object that will generate {@code ResultSet} objects with the
   *     given type and concurrency
   * @throws SQLException if a database access error occurs, this method is called on a closed
   *     connection or the given parameters are not {@code ResultSet} constants indicating type and
   *     concurrency
