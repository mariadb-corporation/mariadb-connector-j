 ...
 * <p>default implementation use system properties `mariadb.user` and `mariadb.pwd`</p>
 * <p>example :
 * `jdbc:mariadb://host/db?identityType=PROPERTY`
 * will use environment variable MARIADB_USER and MARIADB_PWD</p>
 * <p>2 options `userKey` and `pwdKey` permits to indicate which environment variable to use.</p>
 */
/**
 * Authentication using java system properties.
 * <p>default implementation use system properties `mariadb.user` and `mariadb.pwd`</p>
 * <p>example : `jdbc:mariadb://host/db?identityType=PROPERTY`</p>
