...
public class ServerPreparedStatement extends BasePreparedStatement {
   ...
   * @param autoGeneratedKeys must return automatically generated keys?
  ...
  public ServerPreparedStatement(
      ...
      CachedPrepareResultPacket prepareResult)
      throws SQLException {
  ...
  protected void executeInternal() throws SQLException {
    ...
    try (ClosableLock ignore = lock.closeableLock();
      ...
      // Check if prepareResult is invalid (after failover)
      if (prepareResult != null 
          && !prepareResult.isValidForGeneration(con.getContext().getContextGeneration())) {
        prepareResult = null;
      }
      if (prepareResult == null && con.cachePrepStmts())
        prepareResult = (CachedPrepareResultPacket) con.getContext().getPrepareCacheCmd(cmd);
  ...
  private void executePipeline(String cmd) throws SQLException {
    // server is 10.2+, permitting to execute COM_STMT_EXECUTE with statement is -1 (meaning last
    // prepared).
    // send to server COM_PREPARE, followed by COM_STMT_EXECUTE, in one exchange.
    try {
      List<Completion> res =
          con.getClient()
              .execute(
  ...
  private void executeStandard(String cmd) throws SQLException {
    ...
    if (prepareResult == null) {
      if (con.cachePrepStmts()) prepareResult = (CachedPrepareResultPacket) con.getContext().getPrepareCacheCmd(cmd);
      if (prepareResult == null) {
  ...
  protected BatchTypeState executeInternalPreparedBatch() throws SQLException {
    checkNotClosed();
    String cmd = escapeTimeout(sql);
    ...
    // a bit complex:
    // * bulk on INSERT ON DUPLICATE KEY UPDATE can only be use if server permits returning
    // individual results, since affected rows cannot be guessed
    // * if RETURN_GENERATED_KEYS is expressly set, can only be use if server permits returning
    // individual results
    boolean canUseBulk =
    ...
    List<Completion> res =
        con.getClient()
            .executePipeline(
                ...
                closeOnCompletion,
                false);
    // in case of failover, prepare is done in failover, skipping the PREPARE result
  ...
  private void executeBatchStandard(String cmd) throws SQLException {
    ...
    for (Parameters batchParameter : batchParameters) {
      // prepare is in a loop, because if the connection fails, prepare is reset, and need to be re
      // prepared
      // Check if prepareResult is invalid (after failover)
      if (prepareResult != null 
          && !prepareResult.isValidForGeneration(con.getContext().getContextGeneration())) {
   ...
   *
   * @throws SQLException if the number of parameters doesn't correspond to the expected number
   */
