...
public class Pool implements Closeable, PoolMBean {
    ...
    private void removeIdleTimeoutConnection() {
        ...
        while (iterator.hasNext()) {
            ...
            if (globalInfo != null) {
                ...
                if (idleTime > TimeUnit.SECONDS.toNanos(globalInfo.getWaitTimeout() - 45)) shouldBeReleased = true;

                //  idle has reach option maxIdleTime value and pool has more connections than minPoolSiz
                if (timedOut && totalConnection.get() > options.minPoolSize) shouldBeReleased = true;
    ...
    private MariaDbPooledConnection getIdleConnection(long timeout, TimeUnit timeUnit) throws InterruptedException {
        ...
        MariaDbPooledConnection item = (timeout == 0) ? idleConnections.pollFirst() : idleConnections.pollFirst(timeout, timeUnit);
